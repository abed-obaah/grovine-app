...
const BookingDetails = () => {
	// Apply the VehicleDetail type to the item parameter
	const renderVehicleDetail = ({ item }: { item: VehicleDetail }) => (
		<View className="flex-row justify-between mb-2">
			<Text className="text-[12px] font-gilroyRegular">{item.label}</Text>
			<Text className="text-[14px] font-gilroyMedium">{item.value}</Text>
		</View>
	);

	return (
		<>
			<ScrollView className="p-4 bg-white rounded-bl-lg rounded-br-lg">
				{/* Bus Info */}
				<View className="p-4 mb-4">
					...
					<View className="mt-4">
						<View className="flex flex-row justify-between items-center mb-7">
							...
							<TouchableOpacity onPress={() => router.push("/tracking/index")}>
								<Text className="text-blue-600 text-[14px] font-gilroyMedium">
									View route
								</Text>
							</TouchableOpacity>
